...
//{1f665c42-d5a8-4fa7-a75a-a94b1c3b6074}
CLSID CLSID_NetSpeed = { 0x1f665c42, 0xd5a8, 0x4fa7, { 0xa7, 0x5a, 0xa9, 0x4b, 0x1c, 0x3b, 0x60, 0x74 } };
...
STDAPI DllGetClassObject(REFCLSID rclsid, REFIID riid, void **ppv)
{
	...
	if (IsEqualCLSID(CLSID_NetSpeed, rclsid))
	{
		hr = E_OUTOFMEMORY;
...
HRESULT RegisterServer()
{
	...
	StringFromGUID2(CLSID_NetSpeed, szCLSID, ARRAYSIZE(szCLSID));
	...
	if (SUCCEEDED(hr))
	{
		...
		if (ERROR_SUCCESS == RegCreateKeyExW(HKEY_CLASSES_ROOT,
			...
			NULL))
		{
			WCHAR const szName[] = L"ÍøËÙ¼àÊÓÆ÷";
...
HRESULT RegisterComCat()
{
	...
	if (SUCCEEDED(hr))
	{
		...
		hr = pcr->RegisterClassImplCategories(CLSID_NetSpeed, 1, &catid);
		pcr->Release();
	}
	return hr;
}
